...
- **Market data priority:** For options chains and stock quotes, use the TradingView desktop reader (`finance-data-providers:tradingview-reader` skill) FIRST. Fall back to the Futu OpenD API (`futuapi` skill and other `futu-*` skills) for data TradingView cannot provide (options chains/Greeks, historical K-lines, capital flow/distribution, short interest, valuation percentiles, analyst ratings, and community sentiment). If the Futu OpenD gateway is not running, warn the user to start it or run `/install-futu-opend`. Do not substitute yfinance, web search, or guesses.
